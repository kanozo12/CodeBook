	...
	<div class="container">
			...
			<div class="col-10 offset-1">
				...
				<table class="table table-striped">
					...
					<c:forEach var="board" items="${list}">
						<tr>
							<td>${board.id}</td>
							<td><a href="/board/view2/${board.id}${criteria.getQuery(criteria.page)}">${board.title}</a></td>
							<td>${board.writer}</td>
		...
		<div class="row mt-3">
			<div class="col-10 offset-1 text-right">
				<a href="/board/write2" class="btn btn-primary">글작성</a>
			</div>
		</div>
		...
		<nav>
			<ul class="pagination justify-content-center">
				<c:if test="${criteria.prev}">
					<li class="page-item">
						<a href="/board/list2${criteria.getQuery(criteria.start - 1)}" class="page-link" aria-label="Previous"> 
				...
				<c:forEach var="i" begin="${criteria.start}" end="${criteria.end}" step="1">
					<li class="page-item">
						<a href="/board/list2${criteria.getQuery(i)}" class="page-link">${i}</a>
					</li>
				</c:forEach>
	
				<c:if test="${criteria.next}">
					<li class="page-item">
						<a href="/board/list2${criteria.getQuery(criteria.end + 1)}" class="page-link" aria-label="Next"> 
